    ...
    <div class="min-h-screen flex">
        ...
        <aside class="w-[260px] bg-slate-900 flex flex-col fixed h-full z-30 transition-all duration-300">
            ...
            <div class="px-5 py-5 flex items-center gap-3">
                ...
                <span class="text-white font-semibold text-lg tracking-tight">SMS Gateway</span>
            </div>

            <!-- Navigation -->
            <nav class="flex-1 px-3 mt-2 space-y-0.5">
                <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('dashboard') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-slate-400 hover:bg-white/10 hover:text-white' }}">
                ...
                <a href="{{ route('messages.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('messages.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-slate-400 hover:bg-white/10 hover:text-white' }}">
                ...
                <a href="{{ route('contacts.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('contacts.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-slate-400 hover:bg-white/10 hover:text-white' }}">
                ...
                <a href="{{ route('templates.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('templates.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-slate-400 hover:bg-white/10 hover:text-white' }}">
                ...
                <a href="{{ route('campaigns.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('campaigns.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-slate-400 hover:bg-white/10 hover:text-white' }}">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z"/></svg>
                    Kampaniyalar
                </a>
                <a href="{{ route('devices.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('devices.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-slate-400 hover:bg-white/10 hover:text-white' }}">
                ...
                <div class="!mt-4 border-t border-white/10 pt-3">
                    ...
                    <a href="{{ route('settings.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('settings.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-slate-400 hover:bg-white/10 hover:text-white' }}">
            ...
            <!-- User info -->
            <div class="p-4 mx-3 mb-3 rounded-xl bg-white/5">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-lg bg-indigo-600/20 flex items-center justify-center text-sm font-semibold text-indigo-400">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-2 mt-1">
                            <div class="flex-1 h-1.5 bg-white/10 rounded-full overflow-hidden">
                                <div class="h-full bg-emerald-400 rounded-full transition-all" style="width: {{ auth()->user()->sms_limit > 0 ? min((auth()->user()->sms_used / auth()->user()->sms_limit) * 100, 100) : 0 }}%"></div>
                            </div>
                            <span class="text-[10px] text-slate-400 whitespace-nowrap">{{ auth()->user()->sms_used }}/{{ auth()->user()->sms_limit }}</span>
                        </div>
                    </div>
